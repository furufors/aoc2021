...
main = interact $ show . play . parsein
    where
        ...
        play (ns0, bs0) = if winner bs0
                          then score (head ns0) bs0
                          else play . gameStep $ (tail ns0, bs0)
        ...
        gameStep ::  GameStatus -> GameStatus
        gameStep (ns, bs) = (ns, map (map (map (is (head ns)))) bs)
        ...
        score :: Int -> [Board] -> Int
        score n [    ] = error "No winner"
        score n (x:xs) = if solved x
                         then n * getScore x
                         else score n xs
